...
NS_IMETHODIMP
nsClipboardProxy::GetIsProxy(bool* aIsProxy) {
  *aIsProxy = true;
  return NS_OK;
}
...
RefPtr<GenericPromise> nsClipboardProxy::AsyncGetDataWithBrowserCheck(
  ...
  RefPtr<mozilla::dom::BrowserChild> browserChild(aBrowserChild);
  ...
  ContentChild::GetSingleton()
      ...
      ->Then(
          GetMainThreadSerialEventTarget(), __func__,
          /* resolve */
          [promise, transferable, browserChild](
            ...
            // Allow unless content analysis says not to.
            if (browserChild) {
              browserChild->CheckClipboardWithContentAnalysis(dataTransfer,
                                                              promise);
            } else {
