...
mozilla::ipc::IPCResult BrowserParent::RecvDoDragAndDropFilesContentAnalysis(
  ...
  if (NS_FAILED(rv)) {
    aResolver(contentanalysis::MaybeContentAnalysisResult(
        contentanalysis::NoContentAnalysisResult::AGENT_NOT_PRESENT));
    return IPC_OK();
  }
  if (MOZ_LIKELY(!contentAnalysisIsActive)) {
    aResolver(contentanalysis::MaybeContentAnalysisResult(
  ...
  // Start by analyzing the first file, and the
  // ContentAnalysisAdditionalFilesPromiseListener will handle the rest.
  ...
  if (NS_SUCCEEDED(rv)) {
    RefPtr<ContentAnalysisAdditionalFilesPromiseListener> listener =
        new ContentAnalysisAdditionalFilesPromiseListener(
            std::move(aResolver), contentAnalysisPromise, std::move(aFilePaths),
            1, windowInner, std::move(documentURIStringCopy),
...
mozilla::ipc::IPCResult BrowserParent::RecvDoDragAndDropTextContentAnalysis(
    nsString&& aText, DoDragAndDropTextContentAnalysisResolver&& aResolver) {
  nsresult rv;
  mozilla::dom::Promise* contentAnalysisPromise = nullptr;
  nsCOMPtr<nsIContentAnalysis> contentAnalysis =
      mozilla::components::nsIContentAnalysis::Service(&rv);
