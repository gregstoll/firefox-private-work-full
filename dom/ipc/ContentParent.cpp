...
class ContentAnalysisPastePromiseListener
  ...
  ContentAnalysisPastePromiseListener(
      PContentParent::DoClipboardContentAnalysisResolver aResolver)
      : mResolver(aResolver) {}
  virtual void ResolvedCallback(JSContext* aCx, JS::Handle<JS::Value> aValue,
    ...
    mResolver(static_cast<int32_t>(nsIContentAnalysisResponse::ACTION_UNSPECIFIED));
  }
 ...
 private:
  ...
  PContentParent::DoClipboardContentAnalysisResolver mResolver;
};
...
mozilla::ipc::IPCResult ContentParent::RecvDoClipboardContentAnalysis(
    PBrowserParent* aBrowser, const IPCTransferableData& aData,
    DoClipboardContentAnalysisResolver&& aResolver) {
  nsresult rv;
  BrowserParent* browser = nullptr;
  if (aBrowser) {
    browser = BrowserParent::GetFrom(aBrowser);
  }
  ...
  if (browser) {
    ...
    if (MOZ_UNLIKELY(contentAnalysisIsActive)) {
      ...
      rv = nsContentUtils::IPCTransferableDataToTransferable(aData, false, trans, false);
      if (NS_FAILED(rv)) {
        return IPC_OK();
      }
      rv = trans->GetTransferData(kTextMime,
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        //JS::Rooted<JSObject*> global(aes.cx(), contentAnalysisPromise->PromiseObj());
        ...
        //  /* reject */
        //  // TODO - is this OK?
        //  [aResolver](nsresult rv) { aResolver(static_cast<int32_t>(nsIContentAnalysisResponse::ACTION_UNSPECIFIED)); });
      }
    }
  }
  return IPC_OK();
