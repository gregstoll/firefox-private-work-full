...
class ContentAnalysisPromiseListener
  ...
  ContentAnalysisPromiseListener(
      PContentParent::DoDragAndDropContentAnalysisResolver aResolver)
      : mResolver(aResolver) {}
 ...
 private:
  ...
  PContentParent::DoDragAndDropContentAnalysisResolver mResolver;
};
}  // namespace
...
mozilla::ipc::IPCResult ContentParent::RecvDoDragAndDropContentAnalysis(
  ...
  //rv = nsContentUtils::SetDataTransferInEvent(&aEvent);
  if (NS_FAILED(rv)) {
    aResolver(contentanalysis::MaybeContentAnalysisResult(
        contentanalysis::NoContentAnalysisResult::ERROR_OTHER));
    return IPC_OK();
  ...
  emptyFilePath.AssignASCII("a");
  nsCString emptyDigest = "a"_ns;
