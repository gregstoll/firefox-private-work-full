...
std::unique_ptr<Agent> Agent::Create(Config config,
                                     ...
                                     ResultCode* rc) {
  auto agent =
      std::make_unique<AgentWin>(std::move(config), std::move(handler), rc);
  return *rc == ResultCode::OK ? std::move(agent) : nullptr;
}

AgentWin::Connection::Connection(const std::string& pipename,
                                 bool user_specific, AgentEventHandler* handler,
                                 bool is_first_pipe, ResultCode* rc)
    : handler_(handler) {
  *rc = ResultCode::OK;
...
ResultCode AgentWin::Connection::QueueReadFile(bool reset_cursor) {
  ...
  if (!ReadFile(handle_, cursor_, read_size_, &count, &overlapped_)) {
    ...
    if (rc != ResultCode::ERR_IO_PENDING && rc != ResultCode::ERR_BROKEN_PIPE) {
...
ResultCode AgentWin::Connection::NotifyIfError(const char* context,
                                               ResultCode rc, DWORD err) {
...
AgentWin::AgentWin(Config config,
  ...
  *rc = ResultCode::OK;
  ...
  std::string pipename = internal::GetPipeName(configuration().name,
  ...
  for (DWORD i = 0; i < kMinNumListeningPipeInstances; ++i) {
    connections_.emplace_back(std::make_unique<Connection>(
        pipename_, configuration().user_specific, handler(), i == 0, rc));
...
ResultCode AgentWin::HandleOneEvent(std::vector<HANDLE>& wait_handles,
                                    bool* stopped) {
  *stopped = false;
  ...
  DWORD index =
      WaitForMultipleObjects(wait_handles.size(), wait_handles.data(),
  ...
  if (rc != ResultCode::OK) {
    ...
    if (!was_listening && connections_.size() > kMinNumListeningPipeInstances) {
  ...
  if (rc == ResultCode::OK && was_listening && connection->IsConnected()) {
    connections_.emplace_back(std::make_unique<Connection>(
        pipename_, configuration().user_specific, handler(), false, &rc));
  }
