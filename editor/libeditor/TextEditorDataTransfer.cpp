...
void GetRequestingUrlFromDocument(EditorBase::Document* document, nsString& requestingUrl) {
  dom::Element* requestingUrlElement = document->GetElementById(u"requestingUrl"_ns);
    ...
    if (url.Equals(u"chrome://global/content/commonDialog.xhtml")) {
      ...
      if (!requestingUrl.IsEmpty()) {
        ...
        if (MOZ_UNLIKELY(contentAnalysisIsActive)) {
          ...
          rv = transferable->GetTransferData(kTextMime, getter_AddRefs(transferData));
          if (NS_SUCCEEDED(rv)) {
            ...
            nsCOMPtr<nsIContentAnalysisRequest> contentAnalysisRequest =
                new mozilla::contentanalysis::ContentAnalysisRequest(
                    ...
                    false, std::move(emptyDigest),
                    std::move(requestingUrl));
            ...
            if (NS_SUCCEEDED(rv)) {
              ...
              RefPtr<ContentAnalysisPromiseListener> listener =
                  new ContentAnalysisPromiseListener(&shouldAllowContent, contentAnalysisPromise);
