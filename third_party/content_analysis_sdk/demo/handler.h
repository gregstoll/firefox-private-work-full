...
class Handler : public content_analysis::sdk::AgentEventHandler {
 public:
  ...
  Handler(unsigned long delay, const std::string& print_data_file_path,
          const RegexArray& _toBlock = RegexArray(),
          const RegexArray& _toWarn = RegexArray(),
          const RegexArray& _toReport = RegexArray()) :
      toBlock(_toBlock), toWarn(_toWarn), toReport(_toReport),
    ...
    if (event->GetRequest().has_text_content()) {
      ...
      success =
          ReadContentFromFile(event->GetRequest().file_path(),
                              &content);
      if (success) {
        caResponse = DecideCAResponse(content, stream);
      }
#endif
