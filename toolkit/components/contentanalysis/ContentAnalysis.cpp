...
nsCString GenerateRequestToken() {
  static mozilla::Mutex requestTokenMutex{
      "ContentAnalysis.cpp:GenerateRequestToken"};
...
ContentAnalysisRequest::ContentAnalysisRequest(unsigned long aAnalysisType,
                                               ...
                                               nsAString&& aUrl,
                                               unsigned long aResourceNameType)
...
static nsresult ConvertToProtobuf(
  ...
  aOut->set_request_token(
      std::string(requestToken.get(), requestToken.Length()));
...
ContentAnalysis::GetMightBeActive(bool* aMightBeActive) {
  ...
  static bool sIsEnabled =
      Preferences::GetBool("browser.contentanalysis.enabled");
  *aMightBeActive = sIsEnabled;
  return NS_OK;
}

nsresult ContentAnalysis::RunAnalyzeRequestTask(
    RefPtr<nsIContentAnalysisRequest> aRequest, bool aAcknowledgeResponse,
